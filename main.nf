...
process STARALIGN {
    ...
    STAR --genomeDir /opt --outFileNamePrefix STAR --runThreadN 20 --outSAMtype BAM SortedByCoordinate --readFilesCommand zcat --readFilesIn "${pair_id}_R1_001.fastq.gz" "${pair_id}_R2_001.fastq.gz" --limitBAMsortRAM 44000000000 
    """
...
process runAnnotation {
	...
	augustus --species=bug --extrinsicCfgFile=/root/miniconda3/config/extrinsic/extrinsic.M.RM.E.W.P.cfg --hintsfile=$hints --softmasking=on --UTR=on --print_utr=on --codingseq=on $genome > bug_optimized_hints.gff
